"""
GenomeAI
Sequence Extraction Engine
"""

from pyfaidx import Fasta


class SequenceExtractor:

    WINDOW_SIZE = 100

    CHROMOSOME_MAP = {
        "1": "NC_000001.11",
        "2": "NC_000002.12",
        "3": "NC_000003.12",
        "4": "NC_000004.12",
        "5": "NC_000005.10",
        "6": "NC_000006.12",
        "7": "NC_000007.14",
        "8": "NC_000008.11",
        "9": "NC_000009.12",
        "10": "NC_000010.11",
        "11": "NC_000011.10",
        "12": "NC_000012.12",
        "13": "NC_000013.11",
        "14": "NC_000014.9",
        "15": "NC_000015.10",
        "16": "NC_000016.10",
        "17": "NC_000017.11",
        "18": "NC_000018.10",
        "19": "NC_000019.10",
        "20": "NC_000020.11",
        "21": "NC_000021.9",
        "22": "NC_000022.11",
        "X": "NC_000023.11",
        "Y": "NC_000024.10",
        "MT": "NC_012920.1",
    }

    def __init__(self, genome_path):

        self.genome = Fasta(genome_path, sequence_always_upper=True)

    def chromosome_name(self, chromosome):

        chromosome = str(chromosome)

        return self.CHROMOSOME_MAP.get(chromosome)

    def extract(self, chromosome, position):

        chromosome = self.chromosome_name(chromosome)

        if chromosome is None:
            return None

        # ClinVar positions are 1-based, while pyfaidx slices are 0-based.
        # Include WINDOW_SIZE bases on each side and the variant itself.
        position = int(position)
        start = position - 1 - self.WINDOW_SIZE
        end = position + self.WINDOW_SIZE

        if start < 0:
            return None

        try:

            sequence = self.genome[chromosome][start:end].seq.upper()

            expected_length = 2 * self.WINDOW_SIZE + 1
            return sequence if len(sequence) == expected_length else None

        except Exception:

            return None
